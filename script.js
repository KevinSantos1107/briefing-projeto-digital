...
function setupScrollEffects() {
  ...
  // Disabled on mobile/touch: iOS Safari re-rasterizes text on every
  // sub-pixel transform change during momentum scroll → visible shimmer.
  const isTouchOrMobile = window.innerWidth < 1024 ||
    ('ontouchstart' in window) ||
    (navigator.maxTouchPoints > 0);

  if (!REDUCED_MOTION && !isTouchOrMobile) {
    let lastY = -1;
    const onScroll = () => {
      ...
      if (header) {
        /*
         * BUG 1 FIX — arredondamento de valores subpixel
         *
         * CAUSA: y * 0.25 e y * 0.18 produzem frações de px (ex: 3.75px).
         * O Safari iOS re-rasteriza o texto toda vez que o translate
         * sai do pixel-grid inteiro → shimmer/tremor visível.
         *
         * SOLUÇÃO: Math.round() garante que os valores de translate
         * sejam sempre inteiros, mantendo o texto no pixel-grid.
         * O efeito parallax continua visível — a diferença de 1px
         * entre frames adjacentes não é perceptível.
         *
         * A opacidade usa toFixed(3) para evitar strings longas
         * (ex: 0.6999999...) que forçam recalc desnecessário de style.
         */
        header.style.setProperty('--py1', Math.round(y * 0.25) + 'px');
        header.style.setProperty('--py2', Math.round(-y * 0.18) + 'px');
        const inner = header.querySelector('.header-inner');
        if (inner) {
          const t = Math.min(1, y / 300);
          inner.style.setProperty('--ht', Math.round(-y * 0.15) + 'px');
          inner.style.setProperty('--ho', (1 - t * 0.7).toFixed(3));
        }
      }
      updateScrollOffset();
    };
